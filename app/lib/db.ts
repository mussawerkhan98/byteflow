...
export async function getServices(): Promise<DbService[]> {
  try {
    ...
    console.error('Turso services error:', error instanceof Error ? error.message : error)
    return []
  }
}
...
export async function getProjects(): Promise<DbProject[]> {
  try {
    ...
    return result.rows.map((row, index) => ({
      ...
      metrics: safeJsonParse<{ value: string; label: string }[]>(row.metrics, []),
    ...
    console.error('Turso projects error:', error instanceof Error ? error.message : error)
    return []
  }
}
...
export async function getProjectBySlug(slug: string): Promise<DbProject | null> {
  try {
    ...
    console.error('Turso project slug error:', error instanceof Error ? error.message : error)
    return null
  }
}
