...
export default async function ServicePageTemplate({ service }: { service: ServiceData }) {
  ...
  // The page's own hardcoded FAQs don't have a CMS category, so they're
  // labelled with the page's own title; CMS FAQs keep their admin-assigned
  // category. ServiceFAQ turns these into "All / <category>" filter tabs.
  const allFaqs = [
    ...service.faqs.map((f) => ({ ...f, category: shown.title })),
          ...
          <div className="max-w-3xl">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              {service.heroStats.map((stat) => (
                <div
                  key={stat.label}
                  className="card-hover flex flex-col gap-1.5 p-5 rounded-2xl"
                  style={{
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-start">
            ...
            <div className="flex flex-col gap-4">
              {service.benefits.map((benefit) => (
                <div
                  key={benefit.title}
                  className="card-hover flex gap-4 p-6 rounded-2xl transition-all duration-300"
                  style={{
                    background: 'linear-gradient(160deg, rgba(44,205,222,0.06) 0%, var(--bg-surface) 100%)',
                    border: '1px solid rgba(44,205,222,0.15)',
                  }}
            ...
            <div className="relative flex items-start justify-between gap-0">
              ...
              {service.process.map((step) => (
                <div key={step.step} className="relative flex flex-col items-center flex-1 px-4">
                  ...
                  />
                  <div
                    className="card-hover group w-full flex flex-col gap-4 p-6 rounded-2xl transition-all duration-300"
                    style={{
          ...
          <div className="lg:hidden flex flex-col">
            {service.process.map((step, i) => (
              <div key={step.step} className="relative flex gap-5">
                ...
                <div
                  className="card-hover flex flex-col gap-3 p-5 rounded-2xl mb-4 flex-1 transition-all duration-300"
                  style={{
                    background: 'linear-gradient(160deg, rgba(44,205,222,0.06) 0%, var(--bg-surface) 100%)',
                    border: '1px solid rgba(44,205,222,0.13)',
                  }}
